...
def is_market_open() -> bool:
    ...
    return market_status.market == "open" # pyright: ignore[reportAttributeAccessIssue]

def get_all_share_prices_polygon_eod():
    client = RESTClient(polygon_api_key)

    probe = client.get_previous_close_agg("SPY")[0] # pyright: ignore[reportIndexIssue]
    ...
    return {result.ticker: result.close for result in results} # pyright: ignore[reportAttributeAccessIssue]


@lru_cache(maxsize=2)
def get_market_for_prior_date(date: str) -> dict:
    market_data = read_market(date)
    if not market_data:
        market_data = get_all_share_prices_polygon_eod()
        write_market(date=date, data=market_data)
...
def get_share_price_polygon_min(symbol) -> float:
    ...
    return result.min.close or result.prev_day.close # pyright: ignore[reportOptionalMemberAccess, reportReturnType, reportAttributeAccessIssue]
...
def get_share_price(symbol) -> float:
    if polygon_api_key:
        try:
            ...
            print(f"An exception occured while using the polygon API due to {e}.")
