...
DEFAULT_TRACE_TYPE:str = "trace"
DEFAULT_SPAN_TYPE:str = "span"
...
class LogTracer(TracingProcessor):
    ...
    def on_trace_start(self, trace):
        ...
        if name:
            write_log(name=name,type=DEFAULT_TRACE_TYPE, message=f"Started: {trace.name}") # type: ignore

    def on_trace_end(self, trace):
        name = self.get_name(trace)
        if name:
            write_log(name=name,type=DEFAULT_TRACE_TYPE, message=f"Ended: {trace.name}") # type: ignore

    def on_span_start(self, span):
        name = self.get_name(span)
        type = span.span_data.type if span.span_data else DEFAULT_SPAN_TYPE
        if name:
            message = "Started"
            if span.span_data:
